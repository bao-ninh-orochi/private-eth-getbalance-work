...
async fn rpc(http: &reqwest::Client, base: &str, method: &str, params: Value) -> Value {
    ...
    let resp = http.post(base).json(&body).send().await.expect("HTTP request failed");
    assert_eq!(resp.status(), 200, "every JSON-RPC round trip must be HTTP 200 (errors live in the body)");
    resp.json::<Value>().await.expect("response was not valid JSON")
}
...
async fn wait_for_block(http: &reqwest::Client, base: &str, target: u64) {
    ...
    loop {
        ...
        let hex = resp["result"].as_str().expect("eth_blockNumber result must be a string");
        let head = u64::from_str_radix(hex.trim_start_matches("0x"), 16).expect("eth_blockNumber result must be 0x-hex");
        if head >= target {
            return;
        }
        assert!(
...
async fn end_to_end_json_rpc() {
    ...
    for (addr, balance) in &handle.demo_accounts {
        let addr_hex = format!("0x{}", addr.iter().map(|b| format!("{b:02x}")).collect::<String>());
        ...
        assert!(resp.get("error").is_none(), "a successful call must not carry an error object: {resp}");
    ...
    let resp = rpc(&http, &base, "eth_getBalance", json!([random_addr, "latest"])).await;
    assert_eq!(resp["result"], json!("0x0"), "a never-seeded address must return 0x0; full response: {resp}");
    ...
    assert_eq!(chain_id_resp["result"], json!(format!("0x{TEST_CHAIN_ID:x}")));
    ...
    let head_hex = block_number_resp["result"].as_str().expect("eth_blockNumber result must be a string");
    assert!(head_hex.starts_with("0x"), "eth_blockNumber must be 0x-prefixed: {head_hex}");
    let head = u64::from_str_radix(&head_hex[2..], 16).expect("eth_blockNumber must be valid hex");
    assert!(head >= 3, "head must have advanced at least as far as wait_for_block already confirmed, got {head}");

    // ── An unknown method with no proxy configured must be denied,
    // never silently ignored and never forwarded anywhere. ──────────────
    let denied_resp = rpc(&http, &base, "eth_sendTransaction", json!([{"from": random_addr}])).await;
    assert!(denied_resp.get("result").is_none(), "a denied method must never carry a result: {denied_resp}");
    assert_eq!(denied_resp["error"]["code"], json!(-32601), "denied method must be JSON-RPC -32601: {denied_resp}");
    let message = denied_resp["error"]["message"].as_str().expect("error message must be a string");
    assert!(
    ...
    assert_eq!(raw.status(), 200, "even a parse failure must be a clean 200-wrapped JSON-RPC error, not 500");
    let malformed_body: Value = raw.json().await.expect("even the error response must be valid JSON");
