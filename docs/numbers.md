...
**IKPIR build (read before reproducing).** The full-rebuild and answer-latency numbers here were measured on 2026-07-31 against the workspace's then-pinned IKPIR `perf/optimized` tag `v0.1.0-perf` (the `0f3b99b` tree), with the default-on `parallel` feature (rayon matvec/GEMM kernels). The workspace has since moved to `v0.2.0-perf` (see the root `Cargo.toml`) — the SimplePIR error sampler changed, nothing else did (ADR-0046) — and these numbers have not been re-measured against it. A `--no-default-features` build reports substantially slower, single-threaded rebuild/answer times (the sizes and delta-byte figures are unaffected). `xtask bench` prints to stdout by default; pass `--write` to overwrite this file, and only do so from a build against the pinned tag — bump the pin and these numbers together, never separately.
